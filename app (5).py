...
def preprocess_data(df, scaler, tfidf, features_for_splitting):
    ...
    if categorical_cols_to_encode:
        # Align new data columns with training data columns after one-hot encoding
    ...
    # Need to identify which of the 'features_for_splitting' are numerical
    numerical_features_in_splitting = [col for col in features_for_splitting if df[col].dtype in ['int64', 'float64']]
    # Exclude dummy/engineered binary features that shouldn't be scaled (e.g., flags)
    # This requires a more sophisticated way to track which features were originally numerical and scaled
    # For simplicity here, we'll assume numerical_features from the notebook is available or recreate the list carefully
    # Let's assume numerical_features was saved or can be recreated
    # Recreate numerical_features list based on the notebook's definition
    numerical_features_original = ['Claim_Amount', 'Customer_Age', 'Premium_Amount'] # Add other original numerical features
